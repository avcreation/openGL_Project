...
{
	fb.colorTexId = new GLuint[1];
	fb.drawBuffers = new GLenum[1];
	glGenTextures(1, fb.colorTexId);
	fb.colorTexId[0] = c_colorTexId;
	glBindTexture(GL_TEXTURE_2D, fb.colorTexId[0]);
	
	glGenTextures(1, &fb.depthTexId);
	fb.depthTexId = c_depthTexId;
	glBindTexture(GL_TEXTURE_2D, fb.depthTexId);
	...
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + 0, GL_TEXTURE_2D, fb.colorTexId[0], 0);
	fb.drawBuffers[0] = GL_COLOR_ATTACHMENT0 + 0;
